...
Sector::Sector(const Parser& parser) {
    // airline code
    ...
    arr_arp_ = {parser.ParseField<decltype(gen::Airport::arp_cd)>(ser::SecField::kArrArpCd),
                    arr_arp_utc_offset_sign * parser.ParseField<std::chrono::minutes>(ser::SecField::kArrArpUtcOffsetHhMm)};
    ...
    // departure date time
    const auto dep_ymd = parser.ParseField<gen::TpSysClkDays>(ser::SecField::kDepYmd);
    const auto dep_hm = parser.ParseField<gen::DurationMins>(ser::SecField::kDepHhMm);
    dep_dt_utc_ = dep_ymd + dep_hm + file_local_utc_multiplier * dep_arp_.utc_offset_duration;
    ...
    const auto arr_ymd = dep_ymd + arr_day_offset_sign * parser.ParseField<gen::DurationDays>(ser::SecField::kArrDayOffset);
    const auto arr_hm = arr_ymd + parser.ParseField<gen::DurationMins>(ser::SecField::kArrHhMm);
    if (arr_dt_utc_ != arr_hm + file_local_utc_multiplier * arr_arp_.utc_offset_duration) {
...
void Sector::SerializeAlterOrigStr(std::string &sec_str) const {
    ...
    sec_str.replace(arr_ymd_str_format.start_idx, arr_ymd_str_format.length,
                    fmt::format(fmt::runtime(arr_ymd_str_format.str_output_format),
                                std::chrono::system_clock::to_time_t(arr_dt_utc_ + arr_arp_.utc_offset_duration)));
    sec_str[ser::SecFieldStrFormatMap[util::to_underlying(ser::SecField::kSolComp)].start_idx] = comp_idx;
